...
#define setWriteDir()                                                          \
  { *dirSet = 0xFF; } ///< Set PORT for output
#define setReadDir()                                                           \
  { *dirClr = 0xFF; } ///< Set PORT for input
...
#define setWriteDir()                                                          \
  { *dirSet = 0xFFFF; } ///< Set PORT for output
#define setReadDir()                                                           \
  { *dirClr = 0xFFFF; } ///< Set PORT for input
#define write16(d)                                                             \
  {                                                                            \
    *writePort = (d);                                                          \
    WR_STROBE;                                                                 \
...
#define NUM_TIMERS                                                             \
  (sizeof tcList /                                                             \
   sizeof tcList[0]) ///< Number of available timer/counter peripherals
...
Adafruit_TFTDMA::Adafruit_TFTDMA(int8_t tc, int8_t reset, int8_t cs, int8_t cd,
                                 int8_t rd, int8_t wr, int8_t d0,
                                 _EPioType periph) {
...
void Adafruit_TFTDMA::setAddrWindow(int16_t x1, int16_t y1, int16_t x2,
                                    int16_t y2) {
  CS_ACTIVE;
  CD_COMMAND;
...
TFT_framebuffer::TFT_framebuffer(int8_t tc, int8_t reset, int8_t cs, int8_t cd,
                                 int8_t rd, int8_t wr, int8_t d0,
                                 _EPioType periph)
...
void TFT_framebuffer::fillRect(int16_t x1, int16_t y1, int16_t w, int16_t h,
                               uint16_t color) {
...
void TFT_framebuffer::fillScreen(uint16_t color) {
  ...
  uint32_t *fbuf32 = (uint32_t *)framebuf, p2 = TFTWIDTH * TFTHEIGHT / 2,
...
uint16_t *TFT_framebuffer::getBuffer(void) { return framebuf; }

bool TFT_framebuffer::getDirtyRect(int16_t *x1, int16_t *y1, int16_t *x2,
                                   int16_t *y2) {
  *x1 = minx;
  *y1 = miny;
  *x2 = maxx;
...
bool TFT_segmented::begin(void) {
  ...
  descriptor = dma.addDescriptor(NULL,               // move data from here
...
void TFT_segmented::fillRect(int16_t x1, int16_t y1, int16_t w, int16_t h,
                             uint16_t color) {
...
void TFT_segmented::update(int16_t x1, int16_t y1, int16_t x2, int16_t y2,
  ...
  uint16_t *buf[2];
  uint8_t idx = 2;
  int16_t linesRemaining = height, linesThisSegment = maxSegmentLines;
  buf[0] = segmentBuf;
  ...
  while (linesRemaining > 0) {
    if (idx < 2) {
      ...
      descriptor->SRCADDR.reg = (uint32_t)buf[idx] + descriptor->BTCNT.reg;
#elif TFT_INTERFACE == TFT_INTERFACE_16
      descriptor->BTCNT.reg = width * linesThisSegment;
      descriptor->SRCADDR.reg = (uint32_t)buf[idx] + descriptor->BTCNT.reg * 2;
#endif
      dma_busy = true;
      dma.startJob();
      dma.trigger();
    ...
    if (linesRemaining) {
      ...
      linesThisSegment = (*userCallback)(framebuf, linesThisSegment, userData);
...
bool TFT_scanline::begin(void) {
  ...
  for (uint8_t s = 0; s < 2; s++) {
    for (int d = 0; d < TFTWIDTH; d++) {
      ...
      scanline[s].descriptor[d].BTCTRL.bit.BLOCKACT = DMA_BLOCK_ACTION_NOACT;
#if TFT_INTERFACE == TFT_INTERFACE_8
      scanline[s].descriptor[d].BTCTRL.bit.BEATSIZE = DMA_BEAT_SIZE_BYTE;
#elif TFT_INTERFACE == TFT_INTERFACE_16
      scanline[s].descriptor[d].BTCTRL.bit.BEATSIZE = DMA_BEAT_SIZE_HWORD;
#endif
      ...
      scanline[s].descriptor[d].DSTADDR.reg = (uint32_t)writePort;
    }
  }
